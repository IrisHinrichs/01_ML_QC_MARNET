                ...
                for tp in time_spans.index:
                    ...
                    if linds<=ownn_custPar.train_window_size+ddiff: 
                        ...
                        while curr_end < dat_res.index[-1]:
                            ...
                            vis_obs = dat[np.where(vis_mask)]#vis_data.DATA_VALUE
                            vis_data = dat_res.loc[vis_mask]
                            vis_anom_QF3 = vis_obs[np.where(vis_data.QF3.isin([3,4]))] # dat[vis_data.QF3.isin([3,4])]
                            vis_anom_QF1 = vis_obs[np.where(vis_data.QF1.isin([3,4]))] #dat[vis_data.QF1.isin([3,4])]
                            vis_preds = ts_predict[vis_mask]
                            vis_tvec = time_vec[vis_mask]
                            if not np.isnan(ownn_thresh):
                                vis_anom_ownn = dat[np.where(vis_data.ad_ownn>=ownn_thresh)]
                            else:
                            ...
                            if ddiff==0:
                                if not np.isnan(mm_thresh):
                                    vis_anom_mm = dat[np.where(vis_data.ad_mm>=mm_thresh)]
                                else:
                                    vis_anom_mm = np.nan
...
if __name__=='__main__':   
    ...
    # choose_best_model()
    visualize_predictions_anomalies(stations = ['North Sea Buoy III'], params= ['WT'], dlevels=[-3.])
    #main()
